...
class Dataset:
    ...
    def _load_data(self):
        ...
        for prefix in ['train', 'val', 'test']:
            source_file = os.path.join(self.dataset_path, f'input_{prefix}.pkl')
            target_file = os.path.join(self.dataset_path, f'out_{prefix}.pkl')

            source_text = load_data(source_file, self.source_max_length)
            target_text = load_data(target_file, self.target_max_length)
            source_text, target_text = list(map(list,zip(*((input_d, output_d) for input_d, output_d in zip(source_text, target_text) if len(input_d)!=0 and len(output_d)!=0))))
